...
MEDICAL_DISCLAIMER = (
    "⚕️ **Medical Disclaimer:** I am an AI assistant for informational "
    "purposes only. I am not a substitute for professional medical advice, "
    "diagnosis, or treatment. Always consult a licensed dermatologist for "
    "medical concerns."
)
...
Always recommend seeing a dermatologist when:
...
class DermaAIChatbot:
    ...
    def build_context(self, latest_result: Optional[dict] = None) -> str:
        """Build context string from user's latest scan result."""
        if not latest_result:
            return ""
        context = "\n--- USER'S LATEST SCAN RESULT ---\n"
        context += f"Has Acne: {latest_result.get('has_acne')}\n"
        context += f"Acne Types: {', '.join(latest_result.get('acne_types', []))}\n"
        ...
        context += "Use this to give personalized advice when relevant.\n"
        return context

    def format_history(self, chat_history: list) -> list:
        """Convert DB chat history to Groq message format."""
        formatted = []
    ...
    def chat(
        ...
        try:
            # Add scan context to first message
            ...
            messages += self.format_history(chat_history)
            ...
            # Add disclaimer to first message only
            if is_first_message:
