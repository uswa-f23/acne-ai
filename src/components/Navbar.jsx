...
const Navbar = ({ isLoggedIn, onLogout }) => {
  ...
  return (
    <nav className="bg-white/80 backdrop-blur-md shadow-soft sticky top-0 z-50 border-b-2 border-primary-100">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {isLoggedIn ? (
              <div className="flex items-center space-x-4">
                <Link
                  to="/dashboard"
                  className="flex items-center space-x-2 text-neutral-700 hover:text-primary-500 font-medium transition-colors duration-300"
                >
              ...
              <div className="flex items-center space-x-4">
                <Link
                  to="/login"
                  className="text-neutral-700 hover:text-primary-500 font-medium transition-colors duration-300"
                >
                  Login
                </Link>
          ...
          <div className="md:hidden py-6 space-y-4 animate-fadeIn">
            {navLinks.map((link) => (
              <Link
                ...
                className="block text-neutral-700 hover:text-primary-500 font-medium py-2 transition-colors duration-300"
              >
                {link.name}
              </Link>
            ))}
            ...
            {isLoggedIn ? (
              <>
                ...
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center justify-center space-x-2 bg-primary-100 text-primary-600 px-5 py-3 rounded-full hover:bg-primary-200 transition-all duration-300"
                >
                  <LogOut className="w-4 h-4" />
                  <span>Logout</span>
                </button>
