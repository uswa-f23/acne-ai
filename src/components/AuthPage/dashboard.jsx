...
const DashboardPage = () => {
  ...
  return (
    // CHANGE 1: Reduced py-12 to pt-4 pb-12 to remove extra space at top
    <div className="bg-gradient-to-b from-primary-100 to-primary-300 w-full min-h-screen pt-4 pb-12">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-16 xl:px-24 py-8">
          ...
          <div className="card hover:scale-105 transition-transform duration-300">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-display font-bold text-primary-600">{totalScans}</p>
              </div>
          ...
          <div className="card hover:scale-105 transition-transform duration-300">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-display font-bold text-primary-600">{getTrendLabel(trend)}</p>
              </div>
          ...
          <div className="card hover:scale-105 transition-transform duration-300">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-display font-bold text-primary-600">{progressData?.period_days || 30}</p>
              </div>
          ...
          <div className="card hover:scale-105 transition-transform duration-300">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-3xl font-display font-bold text-primary-600">{getAchievement(totalScans)}</p>
              </div>
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary-400 to-secondary-400 flex items-center justify-center">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* CHANGE 3: "Ready for Your Next Analysis" box removed from here */}

            {/* Progress Overview */}
            <div className="card">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-xl font-display font-bold text-neutral-800">Progress Overview</h3>
                  <p className="text-neutral-600 text-sm mt-1">Your skin journey over time</p>
                </div>
                <Link to="/progress" className="text-primary-600 hover:text-primary-500 font-medium text-sm flex items-center space-x-1">
                  <span>View All</span>
              ...
              {progressData?.data_points?.length > 0 ? (
                  ...
                  {progressData.data_points.slice(-3).map((point, i) => (
                    <div key={i} className="flex items-center space-x-4 bg-primary-50 rounded-xl p-3">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-bold text-primary-600">{Math.round(point.severity_score * 100)}%</p>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-2 gap-6 mb-6">
                  <div className="bg-primary-50 rounded-2xl p-4">
                    ...
                    <p className="text-sm text-neutral-600">Upload your first scan!</p>
                  </div>
                </div>
              )}
              <Link to="/progress" className="btn-outline w-full">View Detailed Progress</Link>
            </div>

            {/* Recent Activity */}
            <div className="card">
              <h3 className="text-xl font-display font-bold text-neutral-800 mb-6">Recent Activity</h3>
              {recentResults.length > 0 ? (
                <div className="space-y-4">
                  {recentResults.map((result) => (
                    <div key={result.result_id} className="flex items-center space-x-4 p-4 bg-primary-50 rounded-2xl hover:bg-primary-100 transition-colors">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-neutral-600">
                          {result.analyzed_at ? new Date(result.analyzed_at).toLocaleDateString() : 'Recent'}
                        </p>
                      </div>
                      <Link to={`/detection`} className="text-primary-500 hover:text-primary-600">
          ...
          <div className="space-y-6">
            <div className="card">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h3 className="text-lg font-display font-bold text-neutral-800">Your Treatment Plan</h3>
              </div>
              {latestTreatment ? (
              ...
              ) : <p className="text-sm text-neutral-500 mb-4">No plan available.</p>}
            ...
            <div className="card bg-gradient-to-br from-secondary-100 to-primary-100 border-secondary-200 rounded-xl p-6">
              <div className="flex items-center space-x-3 mb-4">
                ...
                <h3 className="text-lg font-display font-bold text-neutral-800">Ask Our AI Assistant</h3>
              </div>
              <button className="btn-primary w-full text-sm" onClick={() => window.dispatchEvent(new CustomEvent('openChatbot'))}>
                Start Chat
              </button>
            </div>
            ...
            {/* Recommended Actions */}
            <div className="card">
              <h3 className="text-lg font-display font-bold text-neutral-800 mb-4">Recommended Actions</h3>
              
              {/* CHANGE 2: Added mb-3 to "Check product safety" for space */}
              <Link
                to="/product-scanner"
                className="flex items-center justify-between p-3 bg-primary-50 rounded-xl hover:bg-primary-100 transition-colors duration-300 mb-3"
              >
              ...
              <div className="space-y-3">
                <Link to="/detection" className="flex items-center justify-between p-3 bg-primary-50 rounded-xl hover:bg-primary-100 transition-colors">
                  <span className="text-sm font-medium text-neutral-800">Upload new photo</span>
                  <ChevronRight className="w-5 h-5 text-primary-500" />
                </Link>
                <Link to="/progress" className="flex items-center justify-between p-3 bg-primary-50 rounded-xl hover:bg-primary-100 transition-colors">
      ...
      <Link
        to="/detection"
        className="fixed bottom-8 left-8 z-50 flex items-center space-x-2 bg-primary-500 text-white font-semibold px-6 py-4 rounded-full shadow-lg hover:scale-105 transition-all group"
      >
        <Camera className="w-5 h-5" />
        <span>Upload Image</span>
        <ChevronRight className="w-5 h-8 group-hover:translate-x-1 transition-transform" />
      </Link>
    </div>
  );
};
